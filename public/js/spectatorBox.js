function updateSpectatorBox() {
    ...
    $("#spec_name").html(`${p.observer_slot}| ${p.name}`);
    ...
    if (p.weapon_primary.state === "active") {
        $('#spec_ammo').html(`${p.weapon_primary.ammo_clip}/${p.weapon_primary.ammo_reserve}`);
    } else if (p.weapon_secondary.state === "active") {
        $('#spec_ammo').html(`${p.weapon_secondary.ammo_clip}/${p.weapon_secondary.ammo_reserve}`);
    } else {
        $('#spec_ammo').html('');
    }
    ...
    $('#spec_row_one').css("background", `linear-gradient(30deg, ${p.team === "T" ? colors.t : colors.ct} 50%, rgba(0,0,0,1) 130%`);
