function addPlayerBox(observer_slot, right2left = false) {
  ...
  let row1_background = $("<div></div>")
    .addClass("row1-background")
    .prop("id", `row1_background_${id}`);
  row1_background.append(
    $("<div></div>")
      .addClass("row1-teamcolor")
      .prop("id", `row1_teamcolor_${id}`)
  );
  row1_background.append(
    $("<div></div>").addClass("row1-red").prop("id", `row1_red_${id}`)
  );
  container.append(row1_background);

  let row1 = $("<div></div>")
    .addClass("player-container-row one")
    .prop("id", `player_${id}_tr1`);
  row1_background.append(row1);

  let row2 = $("<div></div>")
    .addClass("player-container-row two")
    .prop("id", `player_${id}_tr2`);
  container.append(row2);

  // 1st row
  row1.append($("<div></div>").addClass("health").prop("id", `health_${id}`));
  row1.append(
    $("<div></div>").addClass("player-id").prop("id", `player_id_${id}`)
  );
  row1.append(
    $("<div></div>").addClass("player-name").prop("id", `player_name_${id}`)
  );
  row1.append(
    $("<div></div>").addClass("round-kills").prop("id", `round_kills_${id}`)
  );
  row1.append(
    $("<div></div>").addClass("primary").prop("id", `primary_weapon_${id}`)
  );

  // 2nd row
  row2.append(
    $("<div></div>").addClass("armor-bomb-kit").prop("id", `kevlar_${id}`)
  );

  let money_container = $("<div></div>").addClass("money-container");
  money_container.append($("<div></div>").addClass("dollar").html("$"));
  money_container.append(
    $("<div></div>").addClass("money").prop("id", `money_${id}`)
  );
  ...
  kd_container.append(
    $("<div></div>").addClass("kills").prop("id", `kills_${id}`)
  );
  kd_container.append($("<div></div>").addClass("D").html("D"));
  kd_container.append(
    $("<div></div>").addClass("deaths").prop("id", `deaths_${id}`)
  );
  ...
  nade_container.append(
    $("<div></div>").addClass("nade").prop("id", `n0_${id}`)
  );
  nade_container.append(
    $("<div></div>").addClass("nade").prop("id", `n1_${id}`)
  );
  nade_container.append(
    $("<div></div>").addClass("nade").prop("id", `n2_${id}`)
  );
  nade_container.append(
    $("<div></div>").addClass("nade").prop("id", `n3_${id}`)
  );
  row2.append(nade_container);
  row2.append(
    $("<div></div>").addClass("secondary").prop("id", `secondary_${id}`)
  );
...
function updateHealthBar(id, h, team_t = false, right2left = false) {
  ...
  $(`#row1_teamcolor_${id}`).css(
    "background",
    `linear-gradient(${dir}, ${c1} 30%, ${c2} 100%`
  );
...
function updatePlayerBox(p, right2left = false) {
  ...
  $(`#kevlar_${id}`).empty();
  let kevlar_add = (e) => {
    right2left ? $(`#kevlar_${id}`).append(e) : $(`#kevlar_${id}`).prepend(e);
  };
  let kevlar_add_text = (e) => {
    right2left
      ? $(`#kevlar_${id}`).appendText(e)
      : $(`#kevlar_${id}`).prepend(e);
  };
  if (p.weapon_c4) {
