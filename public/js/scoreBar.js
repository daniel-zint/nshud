function updateTeamInfo() {
  ...
  $("#team_left_avatar").html(
    $("<img></img>")
      .prop(
        "src",
        config.show_team_avatars ? getSafe(() => teamInfo.team1.avatar, "") : ""
      )
      .prop("alt", "")
  );
  ...
  $("#team_right_avatar").html(
    $("<img></img>")
      .prop(
        "src",
        config.show_team_avatars ? getSafe(() => teamInfo.team2.avatar, "") : ""
      )
      .prop("alt", "")
  );
  ...
  if (teamInfo.maps !== undefined && config.show_team_avatars) {
    ...
    const maps_table = $("#maps").html(
      "<th>MAP</th><th>PICK</th><th>WIN</th><th>SCORE</th>"
    );
    for (let i = 0; i < 8; ++i) {
      if (i in teamInfo.maps) {
        const m = teamInfo.maps[i];
        let tr = $("<tr></tr>").append($("<td></td>").text(m.name));
        // pick
        let td_pick = $("<td></td>");
...
function updateScoreBar(data) {
  ...
  if (
    !(
      "phase" in data.phase_countdowns &&
      "phase_ends_in" in data.phase_countdowns
    )
  ) {
  ...
  if (
    timeSeconds !== roundTimer.time &&
    data.phase_countdowns.phase !== "defuse"
  ) {
  ...
  // defuse
  if (
    getSafe(() => data.bomb.state) === "defusing" &&
    data.bomb.countdown != undefined
  ) {
    ...
    $("#defuse")
      ...
      .css(
        "background",
        `linear-gradient(270deg, ${colors.background_default} ${time_perc}%, ${colors.round_defuse_background} ${time_perc}%`
      );
  } else {
    $("#defuse").html("").css("background", "");
  }

  // round wins
  if (data.phase_countdowns.phase === "freezetime" && data?.round_wins) {
    for (const [key, value] of Object.entries(data.round_wins)) {
      $(`#round_wins_${key}`)
        .find("span")
        .css("background-color", value[0] === "t" ? colors.t : colors.ct);
    }
  ...
  $("#maps").css(
    "top",
    data.phase_countdowns.phase === "freezetime" ? "0" : "-40em"
  );
}
